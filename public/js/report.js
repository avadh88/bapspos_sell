$(document).ready(function() {
    ...
    if ($('#product_return_date_filter').length == 1) {
        $('#product_return_date_filter').daterangepicker(dateRangeSettings, function(start, end) {
            $('#product_return_date_filter').val(
                start.format(moment_date_format) + ' ~ ' + end.format(moment_date_format)
            );
            product_return_report.ajax.reload();
            product_return_grouped_report.ajax.reload();
        });
        $('#product_return_date_filter').on('cancel.daterangepicker', function(ev, picker) {
            $('#product_return_date_filter').val('');
            product_return_report.ajax.reload();
            product_return_grouped_report.ajax.reload();
        });
        $('#product_return_date_filter')
            .data('daterangepicker')
            .setStartDate(moment());
        $('#product_return_date_filter')
            .data('daterangepicker')
    ...
    product_return_report = $('table#product_return_report_table').DataTable({
        processing: true,
        serverSide: false,
        aaSorting: [[3, 'desc']],
        ajax: {
            ...
            data: function(d) {
                var start = '';
                var end = '';
                if ($('#product_return_date_filter').val()) {
                    start = $('input#product_return_date_filter')
                        .data('daterangepicker')
                        .startDate.format('YYYY-MM-DD');
                    end = $('input#product_return_date_filter')
                        .data('daterangepicker')
                        .endDate.format('YYYY-MM-DD');
                }
                d.start_date = start;
        ...
        fnDrawCallback: function(oSettings) {
            $('#footer_subtotal').text(
                sum_table_col($('#product_return_report_table'), 'row_subtotal')
            );
            $('#footer_total_sold').html(__sum_stock($('#product_return_report_table'), 'sell_qty'));
            $('#footer_tax').html(__sum_stock($('#product_return_report_table'), 'tax', 'left'));
            __currency_convert_recursively($('#product_return_report_table'));
        },
    });

    product_return_grouped_report = $('table#product_return_grouped_report_table').DataTable({
        processing: true,
        serverSide: false,
        aaSorting: [[1, 'desc']],
        ajax: {
            url: '/reports/product-return-grouped-report',
            data: function(d) {
                var start = '';
                var end = '';
                if ($('#product_return_date_filter').val()) {
        ...
        fnDrawCallback: function(oSettings) {
            $('#footer_grouped_subtotal').text(
                sum_table_col($('#product_return_grouped_report_table'), 'row_subtotal')
            );
            $('#footer_total_grouped_sold').html(
                __sum_stock($('#product_return_grouped_report_table'), 'sell_qty')
            );
            __currency_convert_recursively($('#product_return_grouped_report_table'));
        },
    });
    ...
    $(
        '#product_return_report_form #variation_id, #product_return_report_form #location_id, #product_return_report_form #customer_id'
    ).change(function() {
        product_return_report.ajax.reload();
        product_return_grouped_report.ajax.reload();
    });

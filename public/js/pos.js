$(document).ready(function() {
    ...
    $('select#customer_id').change(function() {
        if($("#add_pos_sell_form").length==1)
        {
            $('table#pos_table tbody').html('');
            $('#return_seva_by_customer_id').val('');
            $('#damage_by_customer_id').val('');
        }
        
    });
    $('select#return_seva_by_customer_id').change(function() {
        if($("#add_pos_sell_form").length==1)
        {
            $('table#pos_table tbody').html('');
            $('#damage_by_customer_id').val('');
    ...
    $('#customer_id').on('select2:select', function(e) {
        ...
        if(customerId==61)
        {
            $("#damage_by_customer_id").prop('disabled',false);
        }
        else
        {
            $("#damage_by_customer_id").prop('disabled',true);
        }
    ...
    let departmentUser = $('#department_user').val();
    ...
    $('#search_product')
        .autocomplete({
            ...
            select: function(event, ui) {
                ...
                document.getElementById('search_product').value = ui.item.sub_sku;
        ...
        .autocomplete('instance')._renderItem = function(ul, item) {
            ...
            if(departmentUser==0 && customerId!=97)
            {
                if (item.enable_stock == 1 && item.qty_available <= 0) {
                    ...
                    if (item.enable_stock == 1 && departmentUser==0) {
    ...
    $('table#pos_table tbody').on('change', 'input.pos_quantity', function() {
        ...
        var restrict_sell_with_sellorder = $('#restrict_sell_with_sellorder').val();
        if(restrict_sell_with_sellorder == '1')
        {
            let locationId  = $('input#location_id').val();
            let customerId  = $('select#customer_id').val();
            let productId   = __read_number(tr.find('input.product_id'));
            let departmentUser = $('#department_user').val();
                
            if(departmentUser==0 && customerId!=97)
            {
                    let status     = pos_restrict_sell(locationId,customerId,productId,entered_qty).then(data => {
                    if(data)
                    {
                    ...
                    {
                        cnf = confirm(LANG.restrict_sell_order);
                        if(cnf)
                        {
                            __write_number(tr.find('input.pos_line_total'), line_total, false, 2);
                            tr.find('span.pos_line_total_text').text(__currency_trans_from_en(line_total, true));
    ...
    $('button#pos-finalize').click(function() {
        ...
        if($('#seva_return').is(':visible')){
            let mobileLength = $('#mobile').val().length;
            if(mobileLength > 0 && (mobileLength < 10 || mobileLength > 12)){
                toastr.warning(LANG.mobile_valid);
                return false;
            } 
        }
    ...
    $('input#search_product_box').on('input',function(e){
        var location_id = $('input#location_id').val();
        if (location_id != '' || location_id != undefined) {
            get_product_suggestion_list(
                $('select#product_category').val(),
    ...
    $(document).on('click', 'div.product_box', function() {
        ...
        else 
        {
            ...
            if(restrict_sell_with_sellorder == '1')
            {
                let locationId  = $('input#location_id').val();
                let customerId  = $('select#customer_id').val();
                let productId   = $(this).data('variation_id');

                let departmentUser   = $('#department_user').val();
                
                if(departmentUser == 0 && customerId!=97)
                {
                    let status     = pos_restrict_sell(locationId,customerId,productId,qty=1).then(data => {
                        if(data)
                        {
                            pos_product_row(productId);
                        }
                        else
                        {
                            cnf = confirm(LANG.restrict_sell_order);
    ...
    $('#fetch-product-by-customer').on('click', function () {
        let location_id = $("#location_id").val();
        //let sell_order = $("#sell_order_no").val();
        let customer_id = $('select#customer_id').val();
        if(!customer_id)
        {
            toastr.error('Please select Customer ID')
        }
        // if(!sell_order)
        // {
        //     toastr.error('Please fill Demand order ID')
...
function get_product_suggestion_list(category_id, brand_id, location_id, url = null) {
    ...
    let search_product_box = document.getElementById('search_product_box').value;
    $.ajax({
        method: 'GET',
        url: url,
        data: {
...
function pos_product_row(variation_id,product_qty=1) {
    ...
    if (add_via_ajax) {
        ...
        let departmentUser = $('#department_user').val();
        var damage_by_customer_id = $('select#damage_by_customer_id').val();
        var return_seva_by_customer_id = $('select#return_seva_by_customer_id').val();
        ...
        $.ajax({
            ...
            data: {
                ...
                damage_by_customer_id:damage_by_customer_id,
                return_seva_by_customer_id:return_seva_by_customer_id,
...
function reset_pos_form(){
    ...
    $("select#customer_id option[value='']").text('Enter Customer name / phone').change();
    $("select#return_seva_by_customer_id option[value='']").text('Enter Supplier name').change();
    
	set_default_customer();
	set_location();
    $('#default_customer_id').val("");
    $('#default_customer_name').val("");
...
function set_location() {
    ...
    if ($('input#location_id').val()) {
        ...
        $('input#product_qty').prop('disabled', true);
    }
...
//restrice sell based on sellorder

function pos_restrict_sell(locationId,customerId,productId,qty)
{
    var data           ={};
    ...
    return new Promise((resolve, reject) => {
        $.ajax({
            method: 'POST',
            url: '/products/restrict_sell',
            data:data,
            dataType: 'json',
            success: function(result) {
                if (result.success == 1) 
                { 
                    let remainQty = ((Number(result.data.soquantity)+Number(result.data.sellreturnquantity)+Number(result.data.sellreturnquantity_genralstore)) - (result.data.sellquantity));
                    if(remainQty >= qty)
                    {
                        resolve(true);
                    }
...
function add_product_pos()
{
    var product_qty = document.getElementById('product_qty').value;
    var enable_stock = document.getElementById('enable_stock').value;
    var qty_available = document.getElementById('qty_available').value;
    ...
    if(document.getElementById('customer_id'))
    {
        ...
        if(customer_id=="")
        {
                toastr.error("Cutomer Name is required");
                return false;
        }
    }
    if(document.getElementById('damage_by_customer_id'))
    {
    ...
    if(product_qty=='')
    {
        ...
        $('input#product_qty').focus().select();
    }
    else
    {
        var restrict_sell_with_sellorder = $('#restrict_sell_with_sellorder').val()
        let departmentUser = $('#department_user').val();
        if (enable_stock != 1 || qty_available > 0) {
            //$(this).val(null);
            if(restrict_sell_with_sellorder == '1')
            {
                let locationId = $('input#location_id').val();
                ...
                if(departmentUser == 0 && customerId!=97)
                {
                    let status     = pos_restrict_sell(locationId,customerId,variation_id,product_qty).then(data => {
                        
                        if(data)
                        {
                            pos_product_row(variation_id,product_qty);
                        }
                        else
                        {
                            cnf = confirm(LANG.restrict_sell_order);
...
{
    var classesNodeList = document.querySelectorAll(".input_quantity");
    var quantity = Array.prototype.slice.call(classesNodeList).map(function(element) {
        return element.value;
    });

    var classesNodeListProduct = document.querySelectorAll(".product_id");
    var product = Array.prototype.slice.call(classesNodeListProduct).map(function(element) {
        return element.value;
    });
    let length =document.querySelectorAll(".product_id");
    let productData = [];
    length.forEach(function(elem,index){
        productData.push({"product":product[index],"quantity":quantity[index]});
        
    });

    let random_no = document.getElementById('random_no').value;
    $.ajax({
        method: 'POST',
        url: '/sells/pos/deamndfromcounter',
        async: false,
        data: {
            product_row: productData,
            srno:random_no

        },
        dataType: 'json',
        success: function(result) {
            toastr.success("Product Sent Sucessfully");
        },
    });
}

...
function __sum_data(table, class_name, label_direction = 'right') {
    var stocks = 0;
    table
        .find('tbody')
        .find('tr')
        .each(function() {
            ...
            if (element.data('orig-value')) {
                var unit_name = element.data('unit');
                stocks += parseFloat(element.data('orig-value'));
            }
        });
    var stock_html = '<p class="text-left"><small>';
    stock_html +='<span class="display_currency" data-is_quantity="true">'+stocks+
                '</span> ' +
                '</br>';

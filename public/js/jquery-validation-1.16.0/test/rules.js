...
QUnit.test( "rules() - internal - input", function( assert ) {
	...
	assert.deepEqual( element.rules(), { required: true, minLength: 1 } );
} );
...
QUnit.test( "rules(), ignore method:false", function( assert ) {
	...
	$( "#testForm1clean" ).validate( {
		rules: {
			firstnamec: { required: false, minLength: 1 }
		}
	} );

	assert.deepEqual( element.rules(), { minLength: 1 } );
} );
...
QUnit.test( "rules(), class and attribute combinations", function( assert ) {
	...
	assert.deepEqual( $( "#v2-i4" ).rules(), { required: true, minLength: 1 } );
	assert.deepEqual( $( "#v2-i5" ).rules(), { required: true, minLength: 1, maxlength: 5, customMethod1: "123" } );
	jQuery.validator.autoCreateRanges = true;
	...
	jQuery.validator.autoCreateRanges = false;
	assert.deepEqual( $( "#v2-i7" ).rules(), { required: true, minLength: 1, customMethod: true } );
...
QUnit.test( "rules(), add and remove", function( assert ) {
	...
	assert.deepEqual( $( "#v2-i5" ).rules(), { required: true, email: true, minLength: 1, maxlength: 5, min: 5, customMethod1: "123" } );
...
QUnit.test( "rules(), add and remove static rules", function( assert ) {
	...
	assert.deepEqual( $( "#lastnamec" ).rules(), { required: true, minLength: 1 } );

	var removedRules = $( "#lastnamec" ).rules( "remove", "required email" );
	assert.deepEqual( $( "#lastnamec" ).rules(), { minLength: 1 } );

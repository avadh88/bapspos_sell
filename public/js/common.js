...
console.log(psm100.end);
console.log(financial_year.end);
...
ranges[LANG.psm100] = [psm100.start,psm100.end];
...
jQuery.validator.addMethod('demand_check', function(value, element) {
    let demand_qty = $(element).data('demand-qty');
    let required_qty= $(element).val().replace(/\,/g,'');
    
    if(Number(required_qty) >0)
    {
        let remain = Number(demand_qty) - Number(required_qty);
        ...
        {
            // $(element).val(0);
            $(element).data('msg-demand_check','Product out of demand')
            return false;
        }
    }
    
    // if(Number(sell_qty)+Number(value) > Number(quota_qty))
    // {
    //     $(element).data('msg-demand_check','Quata-'+quota_qty+', Avliable-'+remain)
    //     return false;
    // }
    // else
    // {

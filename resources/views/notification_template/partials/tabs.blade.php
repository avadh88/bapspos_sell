            ...
            <div class="tab-pane @if($loop->index == 0) active @endif" id="cn_{{$key}}">
                ...
                @if($key == 'new_sale')
                    <div class="form-group">
                        ...
                        <label class="checkbox-inline">
                            {!! Form::checkbox('template_data[' . $key . '][auto_send_whatsapp_message]', 1, $value['auto_send_whatsapp_message'], ['class' => 'input-icheck']); !!} @lang('lang_v1.auto_send_whatsapp_message')
                        </label>
                    </div>
                @endif
                @if($key == 'sell_return_genralstore')
                    <div class="form-group">
                        <label class="checkbox-inline">
                            {!! Form::checkbox('template_data[' . $key . '][auto_send]', 1, $value['auto_send'], ['class' => 'input-icheck']); !!} @lang('lang_v1.autosend_email')
                        </label>

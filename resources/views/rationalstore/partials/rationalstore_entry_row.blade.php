@foreach( $variations as $variation)
    <tr>
        ...
        <td>
            {!! Form::hidden('rationingstore[' . $row_count . '][product_id]', $product->id ); !!}
            {!! Form::hidden('rationingstore[' . $row_count . '][variation_id]', $variation->id , ['class' => 'hidden_variation_id']); !!}
            ...
            @endphp
            {!! Form::text('rationingstore[' . $row_count . '][quantity]', number_format($product_qty, $quantity_precision, $currency_details->decimal_separator, $currency_details->thousand_separator), ['class' => 'form-control input-sm rationalstore_quantity input_number mousetrap', 'required', 'data-rule-abs_digit' => $check_decimal, 'data-msg-abs_digit' => __('lang_v1.decimal_value_not_allowed')]); !!}
            <input type="hidden" class="base_unit_cost" value="{{$variation->default_rationalstore_price}}">
            <input type="hidden" class="base_unit_selling_price" value="{{$variation->default_sell_price}}">

            <input type="hidden" name="rationingstore[{{$row_count}}][product_unit_id]" value="{{$product->unit->id}}">
            @if(!empty($sub_units))
                <br>
                <select name="rationingstore[{{$row_count}}][sub_unit_id]" class="form-control input-sm sub_unit">
        ...
        <td>
            {!! Form::text('rationingstore[' . $row_count . '][purchase_price]',
            number_format($variation->default_purchase_price, $currency_precision, $currency_details->decimal_separator, $currency_details->thousand_separator), ['class' => 'form-control input-sm rationalstore_unit_cost input_number', 'required','readonly']); !!}
        </td>
        ...
        <td class="{{$hide_tax}}">
            <div class="input-group">
                <select name="rationingstore[{{ $row_count }}][rationalstore_line_tax_id]" class="form-control select2 input-sm rationalstore_line_tax_id" placeholder="'Please Select'">
                ...
                {!! Form::hidden('rationingstore[' . $row_count . '][item_tax]', 0, ['class' => 'rationalstore_product_unit_tax']); !!}
        ...
        <td class="{{$hide_tax}}">
            @php
                ...
                if($hide_tax == 'hide'){
                    $dpp_inc_tax = number_format($variation->default_rationalstore_price, $currency_precision, $currency_details->decimal_separator, $currency_details->thousand_separator);
                }

            @endphp
            {!! Form::text('rationingstore[' . $row_count . '][rationalstore_price_inc_tax]', $dpp_inc_tax, ['class' => 'form-control input-sm rationalstore_unit_cost_after_tax input_number', 'required']); !!}
        </td>
        ...
        @if(session('business.enable_lot_number'))
            <td>
                {!! Form::text('rationingstore[' . $row_count . '][lot_number]', null, ['class' => 'form-control input-sm']); !!}
            </td>
        @endif
        @if(session('business.enable_product_expiry'))
            <td style="text-align: left;">
                ...
                <div class="input-group @if($hide_mfg) hide @endif">
                    ...
                    {!! Form::text('rationingstore[' . $row_count . '][mfg_date]', null, ['class' => 'form-control input-sm expiry_datepicker mfg_date', 'readonly']); !!}
                </div>
                ...
                <div class="input-group">
                    ...
                    {!! Form::text('rationingstore[' . $row_count . '][exp_date]', null, ['class' => 'form-control input-sm expiry_datepicker exp_date', 'readonly']); !!}
                </div>
                @else

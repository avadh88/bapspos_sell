...
<tr class="product_row" data-row_index="{{$row_count}}">
	<td>
		...
		<span class="text-link text-info cursor-pointer font_size_15" data-toggle="modal" data-target="#row_edit_product_price_modal_{{$row_count}}">
			{!! $product_name !!}
	...
	@php
	$enable_racks = session()->get('business.enable_racks');
	$enable_row = session()->get('business.enable_row');
	$enable_position = session()->get('business.enable_position');
	@endphp
	@if($enable_racks || $enable_row || $enable_position)
	
	<span>{{$locationRackDetails['rack']}}-{{$locationRackDetails['row']}}-{{$locationRackDetails['position']}}</span>
	@endif
	</td>
	...
	<td>
		...
		<input type="hidden" value="{{$product->category_id}}" 
			name="products[{{$row_count}}][category_id]" class="row_category_id">
		...
		<input type="text" data-min="1" class="form-control pos_quantity input_number mousetrap input_quantity" value="{{@format_quantity($product->quantity_ordered)}}" name="products[{{$row_count}}][quantity]" 
		data-allow-overselling="@if(empty($pos_settings['allow_overselling'])){{'false'}}@else{{'true'}}@endif" 
		@if($allow_decimal) data-decimal=1 @else 
		data-decimal=0 data-rule-abs_digit="true"  
		...
		@if($departmentUser == 0 && $product->enable_stock && empty($pos_settings['allow_overselling']) && !$can_return) 
		...
		data-rule-more_then_zero="true" data-msg-more_then_zero="@lang('lang_v1.more_then_zero')"
		@endif
		
		@if($can_return) 
		data-rule-max-value="{{$can_return}}"  data-qty_available="{{$can_return}}" 
		data-msg-max-value="@lang('validation.custom-messages.quantity_return', ['qty'=> $can_return, 'unit' => $product->unit  ])"  
		data-msg_max_default="@lang('validation.custom-messages.quantity_return', ['qty'=> $can_return, 'unit' => $product->unit  ])" 
		data-rule-more_then_zero="true" data-msg-more_then_zero="@lang('lang_v1.more_then_zero')"
		@endif
		
		@if($departmentUser == 1 && $restrict_sell_with_sellorder==1)data-rule-demand_check="true" data-msg-demand_check="Product out of demand"  data-demand-qty={{$demandQty}} @endif
		>

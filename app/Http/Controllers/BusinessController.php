...
class BusinessController extends Controller
{
    ...
    */
    ...
    public function getBusinessSettings()
    {
        ...
        if (empty($business->genral_store_settings)) {
            $genral_store_settings = $this->businessUtil->defaultGenralStoreSettings();
        } else {
            $genral_store_settings = json_decode($business->genral_store_settings, true);
        }

        if (empty($business->custom_req)) {
            $custom_req = $this->businessUtil->defaultCustomReqSettings();
        } else {
        ...
        $whatsapp_settings = [];
        if (empty($business->whatsapp_settings)) {
            $whatsapp_settings = $this->businessUtil->defaultWhatsappSettings();
        } else {
            $whatsapp_settings = json_decode($business->whatsapp_settings,true);
        }
        ...
        return view('business.settings', compact('business', 'currencies', 'tax_rates', 'timezone_list', 'months', 'accounting_methods', 'commission_agent_dropdown', 'units_dropdown', 'date_formats', 'shortcuts', 'pos_settings','genral_store_settings', 'modules', 'theme_colors', 'email_settings', 'sms_settings', 'mail_drivers', 'allow_superadmin_email_settings','genral_store_settings','whatsapp_settings','custom_req'));
    }
    ...
    public function postBusinessSettings(Request $request)
    {
        ...
        try {
            $business_details = $request->only(['name', 'start_date', 'currency_id', 'tax_label_1', 'tax_number_1', 'tax_label_2', 'tax_number_2', 'default_profit_percent', 'default_sales_tax', 'default_sales_discount', 'sell_price_tax', 'sku_prefix', 'time_zone', 'fy_start_month', 'accounting_method', 'transaction_edit_days', 'sales_cmsn_agnt', 'item_addition_method', 'currency_symbol_placement', 'on_product_expiry',
                'stop_selling_before', 'default_unit', 'expiry_type', 'date_format', 'time_format', 'ref_no_prefixes', 'theme_color', 'email_settings', 'sms_settings','genral_store_settings','whatsapp_settings','custom_req']);
            ...
            $business_details['pos_settings'] = json_encode($pos_settings);
             ...
             $genral_store_settings = $request->input('genral_store_settings');
             
             $default_genral_store_settings = $this->businessUtil->defaultGenralStoreSettings();
            foreach ($default_genral_store_settings as $key => $value) {
                if (!isset($genral_store_settings[$key])) {
                    $genral_store_settings[$key] = $value;
                }
            }
             $business_details['genral_store_settings'] = json_encode($genral_store_settings);
            ...
            $psm_100 = $this->businessUtil->getPSM100Year($business->id);
            $request->session()->put('psm_100', $psm_100);

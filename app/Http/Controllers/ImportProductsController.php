        ...
        try {
            ...
            if ($request->hasFile('products_csv')) {
                ...
                foreach ($imported_data as $key => $value) {
                    ...
                    //Add disposable item
                    $disposable_item = trim($value[34]);
                    if (in_array($disposable_item, [0,1])) {
                        $product_array['disposable_item'] = $disposable_item;
                    } else {
                        $is_valid =  false;
                        $error_msg = "Invalid value for DISPOSABLE ITEM in row no. $row_no";
                        break;
                    }

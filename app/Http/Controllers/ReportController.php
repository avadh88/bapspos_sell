...
class ReportController extends Controller
{
    ...
    public function getStockReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $disposable_filter = request()->get('disposable_filter', null);
            if (!empty($disposable_filter)) {
                $query->where('p.disposable_item', $disposable_filter);
            }
    ...
    public function getproductSellReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $query = TransactionSellLine::join(
                ...
                ->leftjoin('units as u', 'p.unit_id', '=', 'u.id')
                ->leftjoin('users as users', 'users.id', '=', 't.created_by')
                ->where('t.business_id', $business_id)
                ->where('t.type', 'sell')
            ...
            return Datatables::of($query)
                ...
                ->rawColumns(['invoice_no', 'unit_sale_price', 'subtotal', 'sell_qty', 'discount_amount', 'unit_price', 'tax','uname'])
                ->make(true);
        }
    ...
    public function getproductReturnReport(Request $request)
    {
        if (!auth()->user()->can('purchase_n_sell_report.view')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        if ($request->ajax()) {
            $variation_id = $request->get('variation_id', null);
            $query = SellReturnLines::join(
                'transactions as t',
                'sell_return_lines.transaction_id',
                '=',
                't.id'
            )
                ->join(
                    'variations as v',
                    'sell_return_lines.variation_id',
                    '=',
                    'v.id'
                )
                ->join('product_variations as pv', 'v.product_variation_id', '=', 'pv.id')
                ->join('contacts as c', 't.contact_id', '=', 'c.id')
                ->join('products as p', 'pv.product_id', '=', 'p.id')
                ->leftjoin('tax_rates', 'sell_return_lines.tax_id', '=', 'tax_rates.id')
                ->leftjoin('units as u', 'p.unit_id', '=', 'u.id')
                ->where('t.business_id', $business_id)
                ->where('t.type', 'sell_return_genralstore')
                ...
                ->select(
                    ...
                    't.ref_no',
                    't.transaction_date as transaction_date',
                    'sell_return_lines.unit_price_before_discount as unit_price',
                    'sell_return_lines.unit_price_inc_tax as unit_sale_price',
                    DB::raw('(sell_return_lines.quantity) as sell_qty'),
                    'sell_return_lines.line_discount_type as discount_type',
                    'sell_return_lines.line_discount_amount as discount_amount',
                    'sell_return_lines.item_tax',
                    'tax_rates.name as tax',
                    'u.short_name as unit',
                    DB::raw('((sell_return_lines.quantity) * sell_return_lines.unit_price_inc_tax) as subtotal')
                )
                ->groupBy('sell_return_lines.id');

            if (!empty($variation_id)) {
                $query->where('sell_return_lines.variation_id', $variation_id);
            }
            $start_date = $request->get('start_date');
            $end_date = $request->get('end_date');
            if (!empty($start_date) && !empty($end_date)) {
            ...
            return Datatables::of($query)
                ->editColumn('product_name', function ($row) {
                 ...
                 ->editColumn('ref_no', function ($row) {
                     return '<a data-href="' . action('SellReturnGenralstoreController@show', [$row->transaction_id])
                            . '" href="#" data-container=".view_modal" class="btn-modal">' . $row->ref_no . '</a>';
                 })
                ->editColumn('transaction_date', '{{@format_date($transaction_date)}}')
                ->editColumn('unit_sale_price', function ($row) {
                ...
                ->rawColumns(['ref_no', 'unit_sale_price', 'subtotal', 'sell_qty', 'discount_amount', 'unit_price', 'tax'])
                ->make(true);
        }
        ...
        return view('report.product_return_report')
            ->with(compact('business_locations', 'customers'));
    }
     ...
     * @return \Illuminate\Http\Response
     */
    public function getproductReturnGroupedReport(Request $request)
    {
        if (!auth()->user()->can('purchase_n_sell_report.view')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        if ($request->ajax()) {
            ...
            $query =SellReturnLines::join(
                ...
                ->select(
                    ...
                    DB::raw('DATE_FORMAT(t.transaction_date, "%Y-%m-%d") as formated_date'),
                    DB::raw("(SELECT SUM(vld.qty_available) FROM variation_location_details as vld WHERE vld.variation_id=v.id $vld_str) as current_stock"),
                    DB::raw('SUM(sell_return_lines.quantity) as total_qty_sold'),
                    'u.short_name as unit',
                    DB::raw('SUM((sell_return_lines.quantity) * sell_return_lines.unit_price_inc_tax) as subtotal')
                )
                ->groupBy('v.id')
                ->groupBy('formated_date');

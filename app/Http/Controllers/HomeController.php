...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('home.index', compact('date_filters', 'sells_chart_1', 'sells_chart_2', 'widgets','all_locations','categories'));
    }
     ...
     * @return \Illuminate\Http\Response
     */
    public function getProductStockAlertPdfFile()
    {
        try
        {
            $business_id = 1;
            ...
            $products = $query->select(
                ...
                'u.short_name as unit'
                    ...
                    ->groupBy('variation_location_details.id')
                    ->orderBy('stock', 'asc')->get()->toArray();
            view()->share('employee',$products);
            $pdf = PDF::loadView('home.pdf_view', [
                'title' => 'Product Alert quantity'
            ]);
            $fileName = 'product_stock.pdf';
            $filePath = public_path('pdf/'. $fileName);
            $pdf->save($filePath);
            Storage::put('public/pdf/product_stock.pdf',$pdf) ;
            $madiaId= $this->uploadWhatsappMedia($filePath);
            $this->autoSendProductStockAlertOnWhatsapp($madiaId);
        }
        catch(\Exception $e)
        {
            dd($e->getMessage());
    ...
    public function autoSendProductStockAlertOnWhatsapp($madiaId)
    {
        ...
        $mobileNo = '919725205609';
        $data['whatsapp_data']['messaging_product'] = "whatsapp";
        $data['whatsapp_data']['to'] = $mobileNo;
        $data['whatsapp_data']['type'] = "template";
        $data['whatsapp_data']['template']['name']='product_stock_alert';
        $data['whatsapp_data']['template']['language']['code']='en_US';
        $data['whatsapp_data']['template']['components'][0]['type']='header';
        $data['whatsapp_data']['template']['components'][0]['parameters'][0]['type']='document';
        $data['whatsapp_data']['template']['components'][0]['parameters'][0]['document']['id']=$madiaId;
        $data['whatsapp_data']['template']['components'][0]['parameters'][0]['document']['filename']='product-stock.pdf';
        $data['whatsapp_data']['template']['components'][1]['type']='body';
        $data['whatsapp_data']['template']['components'][1]['parameters'][0]['type']='text';
        $data['whatsapp_data']['template']['components'][1]['parameters'][0]['text']=date('d-M-Y');
        

        
        if (!empty($mobileNo)) {
        
            $this->utils->sendMessageWhatsapp($data);
        }
    }

    public function uploadWhatsappMedia($filePath)
    {
        $business_id = 1;
        $business = Business::findOrFail($business_id);
        ...
        $data['whatsapp_data']['messaging_product'] = "whatsapp";
        $data['whatsapp_data']['file'] = $filePath;
        
        $response = $this->utils->uploadWhatsappMedia($data,$filePath);

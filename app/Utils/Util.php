...
class Util
{
    ...
    */
    ...
    {
        $whatsapp_settings = json_decode($data['whatsapp_settings'],true);
        ...
        if ($connected){
            ...
            $client = new Client([
                'headers' => [ 'Content-Type' => 'application/json','Authorization'=>'Bearer '.$whatsapp_settings['user_access_token'] ]
            ]);
            
            $response = $client->post($whatsapp_settings['url'].$whatsapp_settings['version'].'/'.$whatsapp_settings['phone_number_id'].'/messages',
                ['body' => json_encode(
                    $data['whatsapp_data']
                )]
            );
           
            \Log::emergency(json_encode($data['whatsapp_data']));
            fclose($connected);
        }else{
            $is_conn = false; //action in connection failure
        }
    ...
    public function uploadWhatsappMedia($data,$filepath)
    {
        ...
        if ($connected){
            ...
            $response = $client->post($whatsapp_settings['url'].$whatsapp_settings['version'].'/'.$whatsapp_settings['phone_number_id'].'/media',
                [
                    'multipart' => 
                    [
                        [
    ...
    */
    ...
    public function replaceTags($business_id, $data, $transaction)
    {
        ...
        if (!is_object($transaction)) {
            $transaction = Transaction::where('business_id', $business_id)
                            ->with(['contact', 'payment_lines','sell_lines','sellreturngenralstore_lines'])
        ...
        foreach ($data as $key => $value) {
            ...
            //Replace bill_date
            if (strpos($value, '{bill_date}') !== false) {
                $billDate   = $this->format_date($transaction->transaction_date,true);
                $data[$key] = str_replace('{bill_date}', $billDate, $data[$key]);
            }

            //Replace additional_notes
            if (strpos($value, '{additional_notes}') !== false) {
                $additional_notes = $transaction->additional_notes;
                $data[$key]       = str_replace('{additional_notes}', $additional_notes, $data[$key]);
            }

            //Replace product_list
            if (strpos($value, '{product_list}') !== false && $transaction->type == 'sell') {
                $additional_notes = $transaction->additional_notes;
                $productList = '';
                $transactionData = TransactionSellLine::where('transaction_id',$transaction->id)->with('product')->get();
                
                $subset = $transactionData->map(function ($transactionData) {
                    return $transactionData->only(['id','quantity', 'product']);
                });
                
                foreach ($subset as $dataProduct) {
                    ...
                    // else
                    // {
                        $productList .= $dataProduct['product']['name'].": *".$dataProduct['quantity']."*\r\n";
                    // }
                    
                }
                $data[$key]       = str_replace('{product_list}', $productList, $data[$key]);
            }

            //Replace product_list
            if (strpos($value, '{product_list}') !== false && $transaction->type == 'sell_return_genralstore') {
                $additional_notes = $transaction->additional_notes;
                $productList = '';
                $transactionData = SellReturnLines::where('transaction_id',$transaction->id)->with('product')->get();
    ...
    */
    ...
    public function sendMessagePending($business_id, $data, $transaction)
    {
        $productDataPending = $transaction->get()->toarray();
        foreach ($data as $key => $value) {
            //Replace contact name
            if (strpos($value, '{productlist}') !== false) {
                $productData = '';
                foreach($productDataPending as $product)
                {
                    $totalpending = ($product->total_sold) - ($product->total_returned);
                    if($totalpending > 0)
                    {
                        $productData .= $product->product.'('.$totalpending.')';
                    }
                   
                }
                $data[$key] = str_replace('{productlist}', $productData, $data[$key]);
            }
        }
        return $data;
    }
     ...
     * @return array
     */
    public function whatsappMesaage($business_id, $transaction)
    {
        $componentData = [];
        ...
        $headerParamData[0]['type'] = 'text';
        $headerParamData[0]['text'] = $billDate.' /'.$invoice_number;
        ...
        $bodyParamData[0]['type']='text';
        $bodyParamData[0]['text']=$productList;


        $bodyParamData[1]['type']='text';
        $bodyParamData[1]['text']=!is_null($additional_notes) ? $additional_notes : '';



        $componentData[0]['type'] = 'header';
        $componentData[0]['parameters'] = $headerParamData;

        $componentData[1]['type'] = 'body';
        $componentData[1]['parameters'] = $bodyParamData;

        return $componentData;
    }

    public function whatsappMesaageSellReturn($business_id, $transaction)
    {
        $componentData = [];
        ...
        //Replace product_list
        if ($transaction->type == 'sell_return_genralstore') {
            $additional_notes = $transaction->additional_notes;
            $productList = '';
            $transactionData = SellReturnLines::where('transaction_id',$transaction->id)->with('product')->get();
    ...
    public function getCustomerNameBuCustomerId()
    {

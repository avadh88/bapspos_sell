...
class ProductUtil extends Util
{
    ...
    public function updateProductQuantity($location_id, $product_id, $variation_id, $new_quantity, $old_quantity = 0, $number_format = null, $uf_data = true)
    {
        ...
        $productCurrentData = VariationLocationDetails::where('variation_id', $variation_id)
        ->where('product_id', $product_id)
        ->where('location_id', $location_id)->get()->first();
        ...
        if ($qty_difference != 0) {
            ...
            if (empty($variation_location_d)) {
                ...
                if($qty_difference < 0)
                {
                    $variation_location_d->increment('qty_available', $qty_difference);
                }
                else{
                ...
                $productUpdatedData = VariationLocationDetails::where('variation_id', $variation_id)
                ->where('product_id', $product_id)
                ->where('location_id', $location_id)->get()->first();
                if(is_null($productCurrentData))
                {
                    \Log::channel('product_stock')->info("Product name => ".$product->name." SKU =>".$product->sku." Old stock=>0 new purchase Qty=>".$new_quantity." New stock=>".$productUpdatedData->qty_available); 
                }
                else
                {
                    \Log::channel('product_stock')->info("Product name => ".$product->name." SKU =>".$product->sku." Old stock=>".$productCurrentData->qty_available." Sell Return or Purchase Qty=>".$new_quantity." New stock=>".$productUpdatedData->qty_available); 
                }
    ...
    public function decreaseProductQuantity($product_id, $variation_id, $location_id, $new_quantity, $old_quantity = 0)
    {
        ...
        if ($product->enable_stock == 1) {
            ...
            //\Log::channel('product_stock')->info("Product name => ".$product->name." SKU =>".$product->sku." Old stock=>".$productCurrentData->qty_available." Sell Qty=>".$new_quantity." New stock=>".$productUpdatedData->qty_available); 
        }
     ...
     * @return array
     */
    public function getDetailsFromVariation($variation_id, $business_id, $location_id = null, $check_qty = true,$departmentUser=null)
    {
     ...
     * @return array
     */
    public function createOrUpdateSellOrderLines($transaction, $input_data, $currency_details, $enable_product_editing, $before_status = null)
    {   
        $updated_sellorder_lines = [];
        $updated_sellorder_line_ids = [0];
        $exchange_rate = $transaction->exchange_rate;
        ...
        foreach ($input_data as $data) 
        {
            ...
            $new_quantity_f = $this->num_f($new_quantity);
            //update existing sellorder line
            if (isset($data['sellorder_line_id'])) 
            {
                $sellorder_line = SellOrderLines::findOrFail($data['sellorder_line_id']);
                $updated_sellorder_line_ids[] = $sellorder_line->id;
                $old_qty = $this->num_f($sellorder_line->quantity);
            } 
            else 
            {
                //create newly added sellorder lines
                $sellorder_line = new SellOrderLines();
                $sellorder_line->product_id = $data['product_id'];
                $sellorder_line->variation_id = $data['variation_id'];
            }
            ...
            $sellorder_line->quantity = $new_quantity;
            $sellorder_line->pp_without_discount = ($this->num_uf($data['pp_without_discount'], $currency_details)*$exchange_rate) / $multiplier;
            $sellorder_line->discount_percent = $this->num_uf($data['discount_percent'], $currency_details);
            $sellorder_line->purchase_price = ($this->num_uf($data['purchase_price'], $currency_details)*$exchange_rate) / $multiplier;
            $sellorder_line->purchase_price_inc_tax = ($this->num_uf($data['purchase_price_inc_tax'], $currency_details)*$exchange_rate) / $multiplier;
            $sellorder_line->item_tax = ($this->num_uf($data['item_tax'], $currency_details)*$exchange_rate) / $multiplier;
            $sellorder_line->tax_id = $data['sellorder_line_tax_id'];
            $sellorder_line->lot_number = !empty($data['lot_number']) ? $data['lot_number'] : null;
            $sellorder_line->mfg_date = !empty($data['mfg_date']) ? $this->uf_date($data['mfg_date']) : null;
            $sellorder_line->exp_date = !empty($data['exp_date']) ? $this->uf_date($data['exp_date']) : null;
            $sellorder_line->sub_unit_id = !empty($data['sub_unit_id']) ? $data['sub_unit_id'] : null;
            $sellorder_line->sell_order_date = !empty($data['sell_order_date']) ? $this->uf_date($data['sell_order_date']) : null;
            
            $updated_sellorder_lines[] = $sellorder_line;
            //var_dump($updated_sellorder_lines); exit;
            
        }

        
        //unset deleted sellorder lines
        $delete_sellorder_line_ids = [];
        $delete_sellorder_lines = null;
        if (!empty($updated_sellorder_line_ids)) 
        {
            $delete_sellorder_lines = SellOrderLines::where('transaction_id', $transaction->id)
                    ->whereNotIn('id', $updated_sellorder_line_ids)
                        ->get();

            if ($delete_sellorder_lines->count()) 
            {
                foreach ($delete_sellorder_lines as $delete_sellorder_line) 
                {
                    $delete_sellorder_line_ids[] = $delete_sellorder_line->id;

                }
                    //Delete deleted sellorder lines
                    SellOrderLines::where('transaction_id', $transaction->id)
                        ->whereIn('id', $delete_sellorder_line_ids)
                            ->delete();
            }
        }

        //update sellorder lines
        if (!empty($updated_sellorder_lines)) 
        {
            $transaction->sellorder_lines()->saveMany($updated_sellorder_lines);
        }
        
        return $delete_sellorder_lines;
    }   
    /**
    ...
    * @return array
    */
    public function changeSellorderLineUnit($sellorder_line)
    {
        $base_unit = $sellorder_line->product->unit;
        $sub_units = $base_unit->sub_units;

        $sub_unit_id = $sellorder_line->sub_unit_id;
        ...
        if (!empty($sub_unit)) {
            $multiplier = $sub_unit->base_unit_multiplier;
            $sellorder_line->quantity = $sellorder_line->quantity / $multiplier;
            $sellorder_line->pp_without_discount = $sellorder_line->pp_without_discount * $multiplier;
            $sellorder_line->purchase_price = $sellorder_line->purchase_price * $multiplier;
            $sellorder_line->purchase_price_inc_tax = $sellorder_line->purchase_price_inc_tax * $multiplier;
            $sellorder_line->item_tax = $sellorder_line->item_tax * $multiplier;
            $sellorder_line->quantity_returned = $sellorder_line->quantity_returned / $multiplier;
            $sellorder_line->quantity_sold = $sellorder_line->quantity_sold / $multiplier;
            $sellorder_line->quantity_adjusted = $sellorder_line->quantity_adjusted / $multiplier;
        }

        return $sellorder_line;
    }
    /**
     ...
     * @return array
     */
    public function createOrUpdateSellReturnsGenralstoreLines($transaction, $input_data, $currency_details, $enable_product_editing, $before_status = null)
    {   
        $updated_sellreturn_lines = [];
        $updated_sellreturn_line_ids = [0];
        $exchange_rate = $transaction->exchange_rate;
        ...
        foreach ($input_data as $data) 
        {
            ...
            $new_quantity_f = $this->num_f($new_quantity);
            //echo ($data['sellreturn_line_id']); exit;
            //update existing sellorder line
            if (isset($data['sellreturn_line_id'])) 
            {
                $sell_return_genralstore                = SellReturnLines::findOrFail($data['sellreturn_line_id']);
                $updated_sellreturn_line_ids[]          = $sell_return_genralstore->id;
                $delete_sellreturn_line_ids[]           = $sell_return_genralstore->id;
                $old_qty                                = $this->num_f($sell_return_genralstore->quantity);
            } 
            else 
            {
                //create newly added sellorder lines
                $sell_return_genralstore                 = new SellReturnLines();
                $sell_return_genralstore->product_id     = $data['product_id'];
                $sell_return_genralstore->variation_id   = $data['variation_id'];
            }
            ...
            $sell_return_genralstore->quantity               = $new_quantity;
            //$sell_return_genralstore->pp_without_discount    = ($this->num_uf($data['pp_without_discount'], $currency_details)*$exchange_rate) / $multiplier;
            //$sell_return_genralstore->discount_percent       = $this->num_uf($data['discount_percent'], $currency_details);
            $sell_return_genralstore->unit_price             = ($this->num_uf($data['purchase_price'], $currency_details)*$exchange_rate) / $multiplier;
            $sell_return_genralstore->unit_price_inc_tax     = ($this->num_uf($data['purchase_price_inc_tax'], $currency_details)*$exchange_rate) / $multiplier;
            $sell_return_genralstore->item_tax               = ($this->num_uf($data['item_tax'], $currency_details)*$exchange_rate) / $multiplier;
            $sell_return_genralstore->tax_id                 = $data['sellreturn_line_tax_id'];
            //$sell_return_genralstore->lot_number             = !empty($data['lot_number']) ? $data['lot_number'] : null;
            //$sell_return_genralstore->mfg_date               = !empty($data['mfg_date']) ? $this->uf_date($data['mfg_date']) : null;
            //$sell_return_genralstore->exp_date               = !empty($data['exp_date']) ? $this->uf_date($data['exp_date']) : null;
            //$sell_return_genralstore->sub_unit_id            = !empty($data['sub_unit_id']) ? $data['sub_unit_id'] : null;
            $sell_return_genralstore->sell_return_note       = !empty($transaction['additional_notes']) ? $transaction['additional_notes'] : null;
            $sell_return_genralstore->quantity_returned      = $new_quantity;
            
            $updated_sell_return_genralstore_lines[] = $sell_return_genralstore;
        ...
        //unset deleted sellorder lines
        $delete_sell_return_genralstore_line_ids = [];
        $delete_sell_return_genralstore_lines = null;
        if (isset($delete_sellreturn_line_ids)) 
        {
            //Delete deleted sellorder lines
            SellReturnLines::where('transaction_id', $transaction->id)
                ->whereNotIn('id', $delete_sellreturn_line_ids)
                    ->delete();
        }
        else
        {
            SellReturnLines::where('transaction_id', $transaction->id)->delete();
        }
        
        //update sellorder lines
        if (!empty($updated_sell_return_genralstore_lines)) 
        {
            return $transaction->sellreturngenralstore_lines()->saveMany($updated_sell_return_genralstore_lines);
        }

...
class TransactionUtil extends Util
{
    ...
    public function createSellTransaction($business_id, $input, $invoice_total, $user_id, $uf_data = true)
    {
        ...
        $transaction = Transaction::create([
            ...
            'final_total' => $uf_data ? $this->num_uf($invoice_total['total_before_tax']+$invoice_total['tax']) : $invoice_total['total_before_tax']+$invoice_total['tax'],
            ...
            'permanent_sell' => !empty($input['permanent_sell']) ? $input['permanent_sell'] : 0,
            'damage_by_customer_id' => !empty($input['damage_by_customer_id']) ? $input['damage_by_customer_id'] : '',
            'damage' => !empty($input['damage_by_customer_id']) ? 1 : 0,
            'return_seva' => !empty($input['return_seva_by_customer_id']) ? 1 : 0,
            'return_seva_by_customer_id' => !empty($input['return_seva_by_customer_id']) ? $input['return_seva_by_customer_id'] : '',
            'return_seva_by_customer_name' => !empty($input['return_seva_by_customer_name']) ? $input['return_seva_by_customer_name'] : null,
    ...
    public function createOrUpdateSellLines($transaction, $products, $location_id, $return_deleted = false, $status_before = null, $extra_line_parameters = [], $uf_data = true)
    {
        ...
        if (!empty($edit_ids)) {
            ...
            $this->deleteSellLines($deleted_lines, $location_id,$status_before);
     ...
     * @return boolean
     */
    public function deleteSellLines($transaction_line_ids, $location_id,$status_before='')
    {
    ...
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type)
    {
        ...
        {
            $waiter = \App\User::find($transaction->created_by);
        }
        ...
        if ($il->show_customer == 1) {
            ...
            $output['customer_mobile'] = $customer->mobile != '' ? $customer->mobile : '';
        ...
        if ($transaction_type == 'sell') {
            ...
            $details = $this->_receiptDetailsSellLines($lines, $il, $business_details,$location_id);
        ...
        elseif ($transaction_type == 'sell_return_genralstore') {
            $parent_sell = Transaction::find($transaction->id);
            $lines = $parent_sell->sellreturngenralstore_lines;
        ...
        $output['show_rack_details'] = $il->show_rack_details;
        ...
        $output['hide_product_price']       = $il->hide_product_price;
        $output['hide_row_sub_total']       = $il->hide_row_sub_total;
        $output['hide_payment_information'] = $il->hide_payment_information;
     ...
     * @return array
     */
    protected function _receiptDetailsSellLines($lines, $il, $business_details,$location_id='')
    {
        $is_lot_number_enabled = $business_details->enable_lot_number;
        $is_product_expiry_enabled = $business_details->enable_product_expiry;
        ...
        foreach ($lines as $line) {
            ...
            if ($il->show_rack_details == 1) {
                $business_id = session()->get('user.business_id');
                $productUtil = new ProductUtil();
                
                $rackDetails = $productUtil->getRackDetails($business_id, $product->id);
                
                $locationRackDetails = $rackDetails[$location_id]['rack'].'-'.$rackDetails[$location_id]['row'].'-'.$rackDetails[$location_id]['position'];
            }
            $line_array = [
                //Field for 1st column
                'name' => $product->name,
                'variation' => (empty($variation->name) || $variation->name == 'DUMMY') ? '' : $variation->name,
                'rack_details'=>$locationRackDetails,
    ...
    */
    ...
    /**
     * sellorder currency details
     *
     * @param int $business_id
     *
     * @return object
     */
    public function sellorderCurrencyDetails($business_id)
    {
        $business = Business::find($business_id);
        $output = ['sellorder_in_diff_currency' => false,
                    'sellreturn_in_diff_currency' => false,
                    'p_exchange_rate' => 1,
                    'decimal_seperator' => '.',
                    'thousand_seperator' => ',',
                    'symbol' => '',
    ...
    */
     ...
     * @return object
     */
    public function sellreturnCurrencyDetails($business_id)
    {
        $business = Business::find($business_id);
        $output = ['sellreturn_in_diff_currency' => false,
                    'sellreturn_in_diff_currency' => false,
                    'p_exchange_rate' => 1,
                    'decimal_seperator' => '.',
                    'thousand_seperator' => ',',
    ...
    public function checkSellforDamage($variation_id, $business_id, $location_id,$damage_by_customer_id)
    {
        $totalSold = 0;
        ...
        $query = Transaction::join('transaction_sell_lines AS TSL', 'transactions.id', '=', 'TSL.transaction_id')
                ...
                ->where('transactions.status', 'final')
                ->where('TSL.product_id', $variation_id)
                ->where('transactions.contact_id', $damage_by_customer_id)
                ->select(DB::raw('SUM(TSL.quantity) as total_sold'))->get()->first();
        if(!is_null($query->total_sold))
        {
            $totalSold = $query->total_sold;
        }
        return $totalSold;
    }
    
    public function checkTotalPurchase($variation_id, $business_id, $location_id,$return_seva_by_customer_id)
    {
        $quantitySold = 0;
        ...
        $query = Transaction::join('purchase_lines AS PL', 'transactions.id', '=', 'PL.transaction_id')
                ...
                ->where('transactions.location_id', $location_id)
                ->where('transactions.type', 'purchase')
                ->where('PL.product_id', $variation_id)
                ->where('transactions.contact_id', $return_seva_by_customer_id)
                ->select(DB::raw('SUM(PL.quantity) as quantity'))->get()->first();

        if(!is_null($query->quantity))
        {
            $quantitySold = $query->quantity;
        }

        return $quantitySold;
    }

    public function checkSellforReturn($variation_id, $business_id, $location_id,$return_seva_by_customer_id){
        $quantityReturn = 0;

        $query = Transaction::join('transaction_sell_lines AS TSL', 'transactions.id', '=', 'TSL.transaction_id')
                ->where('TSL.product_id', $variation_id)
                ->where('transactions.return_seva_by_customer_id', $return_seva_by_customer_id)
                ->select(DB::raw('SUM(TSL.quantity) as quantity'))->get()->first();

        if(!is_null($query->quantity))
        {
            $quantityReturn = $query->quantity;
        }

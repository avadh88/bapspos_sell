...
class Contact extends Authenticatable
{
        ...
        if ($append_id) {
            if(!empty($selected_contacts_array))
            {
                $all_contacts->select(
                    ...
                    'id'
                    )->whereIn('id',$selected_contacts_array);
            }
            else
            {
                $all_contacts->select(
            ...
            if(!empty($selected_contacts_array))
            {
                $all_contacts->select('id', DB::raw("name as customer"))->whereIn('id',$selected_contacts_array);
            }
            else
            {
     ...
     * @return array users
     */
    public static function customersDropdownWithoutDamage($business_id, $prepend_none = true, $append_id = true)
    {
        $user = \Auth::user();
        $selected_contacts_array = [];
        if(count($user->contactAccess)) 
        ...
        $all_contacts = Contact::where('business_id', $business_id)
                        ->whereIn('type', ['customer', 'both'])->where('id','!=',61);
